...
// Snowpack Plugin
export = function (snowpackConfig: SnowpackConfig | object, pluginOptions: IPluginOptions) {
  ...
  if (imports['*'] === true) {
    ...
    imports = {
      ...Object.keys(require(cwdPkgJson).dependencies || {}).reduce(
        (acc: { [keys: string]: boolean }, dependency: string) => {
          acc[dependency] = true
  ...
  if (Object.keys(imports).length === 0) {
    return { // Fallback for type-safe code
      ...
      async transform ({ contents }: ISnowpackTransformOptions) {
        return contents
      }
    }
  }
  ...
  let buildCdnURL: TBuildCDNUrlFn = getSkypackCdnURL
  ...
  return {
    name: 'snowpack-plugin-import-map',
    async transform ({ contents, isDev, fileExt }: ISnowpackTransformOptions): Promise<string> {
 ...
 * @param imports The `options` property of plugin options
 * @param cache The local cache object
 * @param isDev If project is development mode
 * @param buildCdnURL The function to build CDN url
 * @returns The URL to CDN of package or package name unless available
 */
function rewriteImport (source: string, imports: IImportOptions, cache: ICache, isDev: boolean, buildCdnURL: TBuildCDNUrlFn): string {
